...
class MockGenerator:
    ...
    def __init__(self, seed: int = 0, latency_ms: float = 0.0, timer=None) -> None:
        self.seed = seed
    ...
    def generate(
        ...
        with optional_measure(self._timer, "INFERENCE"):
            out: list[list[str]] = []
            for p in prompts:
                cands: list[str] = []
                for i in range(n):
...
class MockScorer:
    ...
    def score(self, prompts: Sequence[str], responses: Sequence[str]) -> List[float]:
        ...
        with optional_measure(self._timer, "SCORE"):
            out: list[float] = []
...
class MockEmbedder:
    ...
    def __init__(self, dim: int = 64, seed: int = 0, latency_ms: float = 0.0, timer=None) -> None:
        self.dim = dim
        self.seed = seed
        self.latency_ms = latency_ms
        self._timer = timer
    ...
    def encode(self, texts: Sequence[str]) -> List[List[float]]:
        with optional_measure(self._timer, "EMBED"):
            out: list[list[float]] = []
            for t in texts:
